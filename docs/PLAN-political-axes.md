...
| B | **machine parts + comparator drafts done** (2026-08-17) — waiting on ~3.5 h human (verify drafts ~1.5 h, two match reviews ~1 h each) | B1 `validate-political.mjs sample` → `data/validation/political-sample.json` (40 firms) · B2 `comparators.csv` pre-filled from OpenSecrets (org blended 2020–24 + PAC-only committee pages) and Goods Unite Us (24/40; 15 paywalled) as marked drafts — draft ρ: PAC vs PAC-only 0.77 (n=29), pooled vs blended 0.96 (n=40) · B3 template `match-review.csv` (1,290 rows, 154 fuzzy first) · B4 `validate` → ρ/κ → `docs/political-validation.md`. Also fixed while sampling: alias collisions (Boeing/BAPA, BK/BNY, BRK-A/B) and junk `n/a` names had silently dropped Boeing's and Dell's PACs — `sameAs` + name variants + collision log |
